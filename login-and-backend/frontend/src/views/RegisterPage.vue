      ...
      <v-card-text>
        <v-form v-model="valid">
          ...
          <v-text-field v-model="confirmPassword" :rules="confirmPasswordRules" label="Confirm Password" type="password"
            ...
            @click:append="passwordShow = !passwordShow" required></v-text-field>
            <v-btn block elevation="8" @click="register()" :loading="loading" type="submit" color="indigo">
...
export default {
  ...
  methods: {
    async register() {
      const response = await axios.post('/register', { name: this.name, email: this.email, password: this.password });
